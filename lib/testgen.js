 ...
 */
async function generateAndFixTest(sourcePath, { runTest, testDir, relativePath, existingTestCode, existingError }) {
    const { connection, model, maxRetries } = await loadConfig();
    ...
    if (existingTestCode != null && existingError != null) {
        prompts.push({
            ...
            and here is the existing test file at (project-relative): ${testRelative}\n\n${existingTestCode}`,
        });
        prompts.push({
            role: "user",
    ...
    if (attempt < maxRetries) {
        ...
        spinner.fail(`❌ Fail to write test for: ${filename}`);
    }
